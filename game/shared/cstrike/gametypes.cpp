...
// These helpers test if we're on a client and/or server...
...
bool GameTypes::Initialize( bool force /* = false*/ )
{
	...
	{
		// CSSO 2.0 compat: try lowercase filename
		if ( !pKV->LoadFromFile( g_pFullFileSystem, "gamemodes.txt" ) )
		{
...
//	pKV->SaveToFile( g_pFullFileSystem, "maps/gamemodes-pre.txt", "GAME" );
	...
	{
		Warning( "GameTypes: LoadGameTypes failed (non-fatal for CSSO 2.0).\n" );
	}

	// Load the maps.
	if ( !LoadMaps( pKV ) )
	{
		Warning( "GameTypes: LoadMaps failed (non-fatal for CSSO 2.0).\n" );
	}

	// Load the map groups.
	if ( !LoadMapGroups( pKV ) )
	{
		Warning( "GameTypes: LoadMapGroups failed (non-fatal for CSSO 2.0).\n" );
	}

	// Load the bot difficulty levels for Offline games.
	if ( !LoadCustomBotDifficulties( pKV ) )
	{
		Warning( "GameTypes: LoadCustomBotDifficulties failed (non-fatal for CSSO 2.0).\n" );
	}
...
void GameTypes::InitMapSidecars( KeyValues* pKV )
{
	...
	{
		const char* curMap = outList[i].Get();
...
bool GameTypes::LoadGameTypes( KeyValues *pKV )
{
	...
	for ( KeyValues *pKV_GameType = pKV_GameTypes->GetFirstTrueSubKey(); pKV_GameType; pKV_GameType = pKV_GameType->GetNextTrueSubKey() )
	{
		...
		if ( !pTypeNameID || *pTypeNameID == 0 )
		{
			// CSSO 2.0 compat: try lowercase
			pTypeNameID = pKV_GameType->GetString( "nameid" );
		}
		if ( pTypeNameID )
		{
		...
		if ( !pKV_GameModes )
		{
			// CSSO 2.0 compat: try lowercase key
			gameModesEntry = "gamemodes";
			pKV_GameModes = pKV_GameType->FindKey( gameModesEntry );
		}
		if ( pKV_GameModes )
		{
...
bool GameTypes::LoadMaps( KeyValues *pKV )
{
	...
	if ( !pKV_Maps )
	{
		// CSSO 2.0 compat: maps entry is optional
		Warning( "GameTypes: could not find entry %s (non-fatal).\n", mapsEntry );
	}
...
bool GameTypes::LoadMapGroups( KeyValues *pKV )
{
	...
	if ( !pKV_MapGroups )
	{
		// CSSO 2.0 compat: mapGroups is optional
		Warning( "GameTypes: could not find entry %s (non-fatal).\n", mapGroupsEntry );
	}
...
bool GameTypes::LoadCustomBotDifficulties( KeyValues *pKV )
{
	...
	if ( !pKV_BotDiffs )
	{
		// CSSO 2.0 compat: botDifficulty is optional
		Warning( "GameTypes: could not find entry %s (non-fatal).\n", botDifficultyEntry );
	}
...
// Purpose: Get the current game mode UI string.
// -------------------------------------------------------------------------------------------- //
const char *GameTypes::GetCurrentGameModeDescID( void )
{
	GameType *pGameType = GetCurrentGameType_Internal();
	Assert( pGameType );
	if ( !pGameType )
	...
	if ( pGameMode->m_DescID_SP[0] != '\0' )
		return pGameMode->m_DescID_SP;

	return pGameMode->m_DescID;
}

// -------------------------------------------------------------------------------------------- //
// Purpose: Get the given game type UI string.
// -------------------------------------------------------------------------------------------- //
const char *GameTypes::GetGameTypeNameID( int nGameType )
{
	if ( nGameType < 0 || nGameType >= m_GameTypes.Count() )
	{
		Warning( "GamesTypes: GetGameTypeNameID() got an invalid argument (%d). Range [%d,%d].\n", nGameType, 0, m_GameTypes.Count() - 1 );
		return 0;
	}
	return m_GameTypes[nGameType]->m_NameID;
}

// -------------------------------------------------------------------------------------------- //
// Purpose: Get the given game mode UI string.
// -------------------------------------------------------------------------------------------- //
const char *GameTypes::GetGameModeNameID( int nGameType, int nGameMode )
{
	if ( nGameType < 0 || nGameType >= m_GameTypes.Count() )
	{
		Warning( "GamesTypes: GetGameModeNameID() got an invalid argument (%d). Range [%d,%d].\n", nGameType, 0, m_GameTypes.Count() - 1 );
		return NULL;
	}
	GameType *pGameType = m_GameTypes[nGameType];
	Assert( pGameType );
	if ( !pGameType )
	{
		return NULL;
	...
	if ( nGameMode < 0 || nGameMode >= pGameType->m_GameModes.Count() )
	{
		Warning( "GamesTypes: GetGameModeNameID() got an invalid argument (%d). Range [%d,%d].\n", nGameMode, 0, pGameType->m_GameModes.Count() - 1 );
		return NULL;
	}
	GameMode* pGameMode = pGameType->m_GameModes[nGameMode];
	Assert( pGameMode );
	if ( !pGameMode )
	{
		return NULL;
...
// -------------------------------------------------------------------------------------------- //
// Purpose: Get the amount of game types
// -------------------------------------------------------------------------------------------- //
int GameTypes::GetGameTypesCount( void )
{
	return m_GameTypes.Count();
}

// -------------------------------------------------------------------------------------------- //
// Purpose: Get the amount of game modes for the given type
// -------------------------------------------------------------------------------------------- //
int GameTypes::GetGameModesCount( int nGameType )
{
	if ( nGameType < 0 || nGameType >= m_GameTypes.Count() )
	{
		Warning( "GamesTypes: GetGameModesCount() got an invalid argument (%d). Range [%d,%d].\n", nGameType, 0, m_GameTypes.Count() - 1 );
		return 0;
	}
	return m_GameTypes[nGameType]->m_GameModes.Count();
}
...
// -------------------------------------------------------------------------------------------- //
// Purpose: Is this map part of this type and mode
// -------------------------------------------------------------------------------------------- //
bool GameTypes::IsValidMapForTypeAndMode( const char *mapName, const char *gameType, const char *gameMode )
{
	if ( !gameType || !gameMode )
	{
		return false;
	...
	// always true if the game mode doesn't even have any map groups
	if ( pGameMode->m_MapGroupsMP.Count() < 1 && pGameMode->m_MapGroupsSP.Count() < 1 )
	{
		return true;
	}
	...
	for ( int i = 0 ; i < pGameMode->m_MapGroupsMP.Count(); ++i )
	{
		if ( IsValidMapInMapGroup( pGameMode->m_MapGroupsMP[i], mapName ) )
		{
			return true;
		}
	}
	...
	for ( int i = 0 ; i < pGameMode->m_MapGroupsSP.Count(); ++i )
	{
		if ( IsValidMapInMapGroup( pGameMode->m_MapGroupsSP[i], mapName ) )
		{
			return true;
		}
	}
...
// -------------------------------------------------------------------------------------------- //
bool GameTypes::GetMapInfo( const char *mapName )
{
	Map *pMap = GetMap_Internal( mapName );
	if ( !pMap )
	{
...
// -------------------------------------------------------------------------------------------- //
// Purpose: Get UI string for a map.
// -------------------------------------------------------------------------------------------- //
const char *GameTypes::GetMapNameID( const char *mapName )
{
	Map *pMap = GetMap_Internal( mapName );
	if ( !pMap )
	{
...
// -------------------------------------------------------------------------------------------- //
// Purpose: Returns the number of bot difficulties for Offline games (where players choose difficulty).
// -------------------------------------------------------------------------------------------- //
int GameTypes::GetCustomBotDifficultyCount()
{
	return m_CustomBotDifficulties.Count();
}

// -------------------------------------------------------------------------------------------- //
// Purpose: Sets the bot difficulty for Offline Games (where players choose difficulty).
// -------------------------------------------------------------------------------------------- //
const char *GameTypes::GetCustomBotDifficultyNameID( int botDiff )
{
	if ( botDiff < 0 ||
		 botDiff >= m_CustomBotDifficulties.Count() )
	{
	...
	return m_CustomBotDifficulties[botDiff]->m_NameID;
}
